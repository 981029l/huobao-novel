...
⚠️ **核心警告：必须保持修炼体系绝对统一！**
禁止混用不同体系（如：不能同时出现"修仙的练气/筑基"和"玄幻的斗者/神桥"或"异能的S/A/B级"）。请根据小说类型，设计**一套独创且名称统一**的境界系统。

你需要设计**${powerTiers}个大境界**，每个大境界内还有**9个小阶段**（初期/中期/后期，各3层）。
每一阶必须写清：
- 境界名称（风格必须严格统一，如全是大自然意象，或全是星辰意象）
...
6. ⚠️ **必须严格沿用《世界观》中设定的境界体系，绝对禁止中途引入或混用其他体系（如修仙混玄幻）。**
